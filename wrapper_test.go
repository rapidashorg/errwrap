...
func Test_errorWrapper_getCallLine(t *testing.T) {
	type fields struct {
		code          int
		codeString    string
		message       string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got := e.getCallLine(tt.args.offset); (got != "") != tt.wantNotEmpty {
				t.Errorf("errorWrapper.getCallLine() = %v, wantNotEmpty %v", got, tt.wantNotEmpty)
			}
		})
	}
}
